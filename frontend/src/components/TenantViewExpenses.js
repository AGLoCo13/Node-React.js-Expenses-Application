...
import { FaHome, FaInfoCircle, FaMoneyBillWave, FaFileInvoiceDollar, FaHistory, FaFire, FaCalendarAlt, FaEuroSign, FaFileAlt } from 'react-icons/fa';
import DashboardLayout from './DashboardLayout';
import { toast } from 'react-toastify';
import 'bootstrap/dist/css/bootstrap.min.css';

function TenantViewExpenses() {
  ...
  const fetchExpenses = async () => {
    try {
      ...
      const response = await axios.get('/api/tenant/building-expenses', {
      ...
      if (error.response?.status === 404) {
        toast.info('No expenses found for your building');
      } else {
        toast.error('Error fetching building expenses');
      }
      setLoading(false);
    }
  };
  ...
  return (
    <DashboardLayout
      ...
      userName="Tenant"
      userRole="Tenant"
      dashboardTitle="Building Expenses"
    >
      {/* Page Header */}
      <div style={{ marginBottom: '2rem' }}>
        ...
        <p style={{ color: '#64748b', fontSize: '1rem' }}>
          View all expenses uploaded by your building administrator
        </p>
      </div>
          ...
          <div style={{ backgroundColor: 'white', borderRadius: '0.75rem', padding: '1.5rem', boxShadow: '0 1px 3px 0 rgba(0, 0, 0, 0.1), 0 1px 2px 0 rgba(0, 0, 0, 0.06)' }}>
            ...
            {expenses.length > 0 ? (
              ...
              <div style={{ textAlign: 'center', padding: '3rem', color: '#64748b' }}>
                ...
                <p>Your building administrator hasn't uploaded any expenses yet</p>
              </div>
            )}
          </div>
        </>
...
export default TenantViewExpenses;
