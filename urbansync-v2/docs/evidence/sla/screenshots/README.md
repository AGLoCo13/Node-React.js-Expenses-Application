...
| `2026-09-20-113604-grafana-app-tail-breach.png` | 11:36:04 | 18 seconds later, the tail crosses the SLO: reads p99 1.017 s (amber), login p99 3.954 s. Shows the tier thresholds firing. **Captured while the Silver p99 target was still 3 s**, so login reads red here; the target was widened to 5 s after this run, and the same value would now render amber. |
...
- **p99 login (Silver)** covers `POST /api/login`, slower by design because bcrypt cost 10
  burns about 75 ms of CPU per call. Amber at 3 s, red at the 5 s target.
